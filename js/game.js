...
const SPLASH_SPAWN_THRESHOLD_SPEED = 0.08; // Lower threshold
const SPLASH_MAX_PARTICLES = 350;         // Even more particles
const SPLASH_BASE_LIFETIME = 1.4;         // Longer lifetime
const SPLASH_PARTICLE_SIZE = 0.65;        // MUCH Larger size
const SPLASH_BASE_OPACITY = 0.7;          // Slightly higher base opacity again?
const SPLASH_SPAWN_RATE_SCALE = 25;       // Significantly higher spawn rate
const SPLASH_SIDE_OFFSET = 1.1;           // Keep similar side offset
const SPLASH_VERTICAL_OFFSET = 0.3;       // Higher spawn point
const SPLASH_BACK_OFFSET = 0.3;           // Slightly less forward/more central?
const SPLASH_INITIAL_VEL_SIDE_MIN = 1.2;  // Adjusted side velocity
const SPLASH_INITIAL_VEL_SIDE_SCALE = 3.0; // More scaling
const SPLASH_INITIAL_VEL_UP_MIN = 2.5;     // More base upward
const SPLASH_INITIAL_VEL_UP_SCALE = 2.5;   // More upward scaling
const SPLASH_GRAVITY = 4.5;
const SPLASH_DRAG = 0.25;                  // Slightly more drag

const PHYSICS_DRAG_FACTOR = 0.98;
// --- MORE CLOUDS ---
const CLOUD_COUNT = 30; // Increased count
const CLOUD_MIN_Y = 40; const CLOUD_MAX_Y = 70; // Slightly higher max?
const CLOUD_AREA_RADIUS = 900; // Wider area
const LARGE_CLOUD_PROBABILITY = 0.2; // Chance for a cloud to be large
const LARGE_CLOUD_SCALE_MULTIPLIER = 2.5; // How much bigger large clouds are
...
const oceanMaterial = new THREE.MeshPhongMaterial({ color: 0x005577, shininess: 100, specular: 0x00bbff, map: waterTexture, normalMap: waterNormalMap, normalScale: new THREE.Vector2(0.2, 0.2), side: THREE.FrontSide }); // Opaque, FrontSide
...
const oceanAnimation = { time: 0, scrollSpeedX: 0.015, scrollSpeedZ: 0.01, normalScrollSpeedX: 0.02, normalScrollSpeedZ: 0.015 }; // Texture scroll speeds
...
function createCloud() {
    ...
    const puffMat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.5 + Math.random() * 0.25, depthWrite: false });
...
function updateOfflineEffects(deltaTime) { // Handles animations/UI updates
    ...
    oceanAnimation.time += deltaTime; // Keep time flowing
    // Use modulo to prevent offsets getting huge, improves precision slightly
    waterTexture.offset.x = (waterTexture.offset.x + oceanAnimation.scrollSpeedX * deltaTime) % 1;
    waterTexture.offset.y = (waterTexture.offset.y + oceanAnimation.scrollSpeedZ * deltaTime) % 1;
    waterNormalMap.offset.x = (waterNormalMap.offset.x + oceanAnimation.normalScrollSpeedX * deltaTime) % 1;
    waterNormalMap.offset.y = (waterNormalMap.offset.y + oceanAnimation.normalScrollSpeedZ * deltaTime) % 1;
