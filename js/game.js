...
// Increased ambient/sky light, reduced direct sun light
const hemiLight = new THREE.HemisphereLight(0xB1E1FF, 0xB97A20, 1.0); // Intensity up to 1.0
scene.add(hemiLight);
const sunLight = new THREE.DirectionalLight(0xffffff, 1.0); // Intensity down to 1.0
sunLight.position.set(100, 150, 100); sunLight.castShadow = true; /* Shadow map settings */ scene.add(sunLight);

// --- Ocean --- (Texture Animation Slowed, Waves Disabled)
...
const oceanMaterial = new THREE.MeshPhongMaterial({
    color: 0x005577, shininess: 100, specular: 0x00aaff, map: waterTexture,
    normalMap: waterNormalMap, normalScale: new THREE.Vector2(0.3, 0.3), // Keep normal scale high for texture
...
// --- SLOWER SCROLL SPEED ---
const oceanAnimation = { time: 0, scrollSpeedX: 0.002, scrollSpeedZ: 0.001, normalScrollSpeedX: 0.004, normalScrollSpeedZ: 0.0025 };
...
islandTexture.repeat.set(4, 4); // Initial repeat

// --- Utility Functions ---
function createShip(isNPC = false) { /* ... (no changes) ... */ }
...
function createIsland(x, z, size, scaleX = 1, scaleZ = 1, rotation = 0, isLarge = false) {
    ...
    const islandBaseTexture = islandTexture.clone(); islandBaseTexture.needsUpdate = true; islandBaseTexture.repeat.set(Math.max(2, Math.round(size * scaleX * 0.1)), Math.max(2, Math.round(size * scaleZ * 0.1)));
    const baseMat = new THREE.MeshPhongMaterial({ color: 0x8B4513, map: islandBaseTexture, shininess: 10, specular: 0x111111 });
    ...
    // Trees & Huts
    // --- ADDED: Small chance for trees on REGULAR islands ---
    const treeCount = isLarge ? Math.floor(size * 0.9) + 6 : (Math.random() < 0.4 ? Math.floor(Math.random() * 2) + 1 : 0); // 40% chance of 1-2 trees on small islands
    if (treeCount > 0) {
        for (let i = 0; i < treeCount; i++) {
            const tree = createPalmTree(); // Uses updated createPalmTree
            const angle = Math.random() * Math.PI * 2;
            const treeDistX = (Math.random() * size * scaleX * 0.85);
            const treeDistZ = (Math.random() * size * scaleZ * 0.85);
            tree.position.set(Math.cos(angle) * treeDistX, islandHeight, Math.sin(angle) * treeDistZ);
            tree.rotation.y = Math.random() * Math.PI * 2;
            tree.position.applyAxisAngle(new THREE.Vector3(0,1,0), rotation);
            islandGroup.add(tree);
        }
    }
    // Huts only on large islands
    if (isLarge) {
        const hutCount = Math.floor(size * 0.15) + 1;
        for (let i = 0; i < hutCount; i++) { const hut = createHut(); const angle = Math.random() * Math.PI * 2; const hutDistX = (Math.random() * size * scaleX * 0.7); const hutDistZ = (Math.random() * size * scaleZ * 0.7); hut.position.set(Math.cos(angle) * hutDistX, islandHeight, Math.sin(angle) * hutDistZ); hut.rotation.y = Math.random() * Math.PI * 2; hut.position.applyAxisAngle(new THREE.Vector3(0,1,0), rotation); islandGroup.add(hut); }
    }
...
function createPalmTree() {
    ...
    const trunkHeight = 4 + Math.random() * 3; // Taller trunks
    const trunkRadius = 0.25 + Math.random() * 0.1; // Slightly thicker?
    const trunkGeo = new THREE.CylinderGeometry(trunkRadius * 0.8, trunkRadius, trunkHeight, 6); // Tapered
    const trunkMat = new THREE.MeshStandardMaterial({ color: 0x8B4513, roughness: 0.8, flatShading: true }); const trunk = new THREE.Mesh(trunkGeo, trunkMat); trunk.position.y = trunkHeight / 2; trunk.castShadow = true; treeGroup.add(trunk);
    const leafCount = 6 + Math.floor(Math.random() * 3); // Maybe more leaves
    const leafLength = 2.0 + Math.random(); // Longer leaves
    const leafWidth = leafLength * 0.8;
    const leafGeo = new THREE.ConeGeometry(leafWidth / 2 , leafLength, 5); // Adjust cone based on length
    const leafMat = new THREE.MeshStandardMaterial({ color: 0x228B22, roughness: 0.7, flatShading: true });
    for (let i = 0; i < leafCount; i++) { const leaf = new THREE.Mesh(leafGeo, leafMat); leaf.position.y = trunkHeight - 0.3; /* Attach near top */ const angle = (i / leafCount) * Math.PI * 2 + (Math.random() - 0.5) * 0.4; const tilt = Math.PI / 3.5 + (Math.random() - 0.5) * 0.4; /* More pronounced droop */ leaf.position.x = Math.cos(angle) * 0.4; /* Spread start point slightly */ leaf.position.z = Math.sin(angle) * 0.4; leaf.rotation.x = tilt * Math.sin(angle); leaf.rotation.z = -tilt * Math.cos(angle); leaf.rotation.y = -angle; leaf.castShadow = true; treeGroup.add(leaf); }
    return treeGroup;
}
function createHut() { /* ... (no changes needed from last version) ... */ }

// --- ADJUSTED MINIMAP ISLAND MARKER COLOR ---
function createMinimapMarker(color, size = 6, isIsland = false, scaleX = 1, scaleZ = 1) {
    ...
    if (isIsland) {
        ...
        markerColor = 0xD2B48C; // Use Tan color for islands on minimap for better visibility
    } else {
